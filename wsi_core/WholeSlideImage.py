...
def _infer_stride_from_overlap_or_coords(coords, patch_size, overlap=None):
    if overlap is not None:
        sx = max(1, int(round(patch_size[0] * (1 - overlap))))
        sy = max(1, int(round(patch_size[1] * (1 - overlap))))
        return sx, sy
    coords = np.asarray(coords, dtype=np.int32)
    xs = np.unique(coords[:, 0]); ys = np.unique(coords[:, 1])
    dx = np.diff(np.sort(xs)); dy = np.diff(np.sort(ys))
    sx = int(np.median(dx[dx > 0])) if np.any(dx > 0) else int(patch_size[0])
    sy = int(np.median(dy[dy > 0])) if np.any(dy > 0) else int(patch_size[1])
    return max(1, sx), max(1, sy)

import numpy as np

def _infer_stride(coords, patch_size):
    coords = np.asarray(coords, dtype=np.int32)
    xs = np.unique(coords[:, 0]); ys = np.unique(coords[:, 1])
    dx = np.diff(np.sort(xs)); dy = np.diff(np.sort(ys))
    sx = int(np.median(dx[dx > 0])) if np.any(dx > 0) else int(patch_size[0])
...
def _nearest_axis_neighbors(coords, i, tol_x, tol_y):
    x, y = coords[i]
    dx = coords[:, 0] - x
    dy = coords[:, 1] - y

    cand = np.where((np.abs(dy) <= tol_y) & (dx > 0))[0]
    right = cand[np.argmin(dx[cand])] if cand.size else None

    cand = np.where((np.abs(dy) <= tol_y) & (dx < 0))[0]
    left = cand[np.argmax(dx[cand])] if cand.size else None

    cand = np.where((np.abs(dx) <= tol_x) & (dy > 0))[0]
    down = cand[np.argmin(dy[cand])] if cand.size else None

    cand = np.where((np.abs(dx) <= tol_x) & (dy < 0))[0]
    up = cand[np.argmax(dy[cand])] if cand.size else None

    nbrs = []
    for j in [right, left, down, up]:
        if j is not None and j != i:
            nbrs.append(int(j))
        else:
            nbrs.append(None)
    return nbrs  # [right, left, down, up]

def reassign_locally_isolated_patches_v2(coords, region_indices, patch_size,
                                         iterations=3, mode='argmax'):
    try:
        import torch
        is_torch = isinstance(region_indices, torch.Tensor)
        if is_torch:
            dev = region_indices.device
            arr = region_indices.detach().cpu().numpy().astype(np.int32)
        else:
            arr = np.asarray(region_indices, dtype=np.int32)
    except Exception:
        is_torch = False
        arr = np.asarray(region_indices, dtype=np.int32)
    ...
    sx, sy = _infer_stride(coords, patch_size)
    tol_x = max(1, int(0.5 * sx))
    tol_y = max(1, int(0.5 * sy))
    ...
    for _ in range(max(1, int(iterations))):
        ...
        for i in range(N):
            nbr_idx = _nearest_axis_neighbors(coords, i, tol_x=tol_x, tol_y=tol_y)
            nbr_idx = [j for j in nbr_idx if j is not None]
            if not nbr_idx:
                continue

            nbr_labels = arr[nbr_idx]
            cur = arr[i]

            if cur in nbr_labels:
                continue

            uniq, counts = np.unique(nbr_labels, return_counts=True)
            if mode == 'prob':
                probs = counts / counts.sum()
                target = int(rng.choice(uniq, p=probs))
            else:
                mf = counts.max()
                cands = uniq[counts == mf]
                ...
                if len(cands) == 1:
                    ...
                    for j in nbr_idx:
                        if arr[j] in cands:
                            dx = coords[j, 0] - x
                            dy = coords[j, 1] - y
                            dists.append((abs(dx) + abs(dy), arr[j]))
                    target = int(sorted(dists, key=lambda t: t[0])[0][1])
    ...
    if is_torch:
        import torch
        return torch.as_tensor(arr, device=dev, dtype=region_indices.dtype)
    return arr

def add_region_outlines(img, coords, patch_size, region_indices, thickness=2, include_outer_border=True):
    H, W = img.shape[:2]
    ...
    try:
        ...
        if isinstance(region_indices, torch.Tensor):
            region_indices = region_indices.detach().cpu().numpy()
    except Exception:
        pass
    region_indices = np.asarray(region_indices, dtype=np.int32)

    coords = np.asarray(coords, dtype=np.int32)
    ph, pw = int(patch_size[1]), int(patch_size[0]) 

    for (x, y), rid in zip(coords, region_indices):
        y0, y1 = max(0, y), min(H, y + ph)
        x0, x1 = max(0, x), min(W, x + pw)
        if y0 < y1 and x0 < x1:
            labels[y0:y1, x0:x1] = rid

    boundary = np.zeros((H, W), dtype=np.uint8)
    diff = np.zeros((H, W), dtype=bool)
    diff[1:, :]   |= (labels[1:, :]   != labels[:-1, :])
    diff[:, 1:]   |= (labels[:, 1:]   != labels[:, :-1])
    diff[1:, 1:]  |= (labels[1:, 1:]  != labels[:-1, :-1])
    diff[1:, :-1] |= (labels[1:, :-1] != labels[:-1, 1:])
    boundary[diff] = 255

    if not include_outer_border:
        bg = (labels == -1)
        rm = np.zeros_like(boundary, dtype=bool)
        rm |= bg
        rm[1:, :]   |= bg[:-1, :]
        rm[:-1, :]  |= bg[1:, :]
        rm[:, 1:]   |= bg[:, :-1]
...
class WholeSlideImage(object):
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
                   ...
                   max_size=None,
                   custom_downsample = 1,
                   cmap='coolwarm',region_indices = None):
        ...
        if region_indices is not None:
            region_indices = reassign_locally_isolated_patches_v2(
                    coords=coords,
